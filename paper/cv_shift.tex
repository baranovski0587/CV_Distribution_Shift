...
Let $(\mathcal X,\mathcal B_{\mathcal X})$ and $(\mathcal Y,\mathcal B_{\mathcal Y})$ be measurable spaces,
and for each $r$ let $\cD_r$ be a probability measure on
...
$(X_t,Y_t) \stackrel{\text{i.i.d.}}{\sim} \cD_r$ for $t\in\mathcal T_r$,
with $\cD_r \neq \cD_s$ generally.
...
\[
Q \;:=\; \sum_{r\in I} w_r \cD_r,
\qquad
w_r \;:=\; \frac{n_r}{\sum_{j\in I} n_j}.
\]
Let $P$ denote the deployment distribution (typically $P=\cD_R$).
...
Moreover, assume that for each $\lambda\in\Lambda$,
\[
...
\;:=\;
\frac{1}{K} \sum_{k=1}^{K} \cR(\lambda;\, Q^{(k)} \to \cD_{v_k}),
\]
where $\cR(\lambda; Q^{(k)} \to \cD_{v_k}) := \E_{(X,Y) \sim \cD_{v_k}}[\ell(f^\star_{\lambda, Q^{(k)}}(X), Y)]$.
...
the validation distributions $\cD_{v_k}$ are exchangeable and temporal CV
